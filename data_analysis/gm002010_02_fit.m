...
pathFile = 'data/processed/gm002010.mat';
load(pathFile);
...
yData = data.y/max(data.y);
% for jj = 1:numel(data.y(1, :))
Fit = esfit(yData, @pepper, {Sys, Exp}, {SysVary}, FitOpt);
[Fit.yFit, Fit.SysBest] = simulatecomponentsfit(Fit);
clf
plot(data.x, yData, data.x, Fit.fit, data.x, ...
    Fit.yFit(1, :), data.x, Fit.yFit(2, :))
fitResults = sprintf("%f\n%f\n%f\n%f\n%f\n%f\n%f", ...
    Fit.pfit(1), Fit.pfit(2), Fit.pfit(3), ...
    Fit.pfit(4), Fit.pfit(5));
% fitResults = sprintf("%f\n%f", Fit.pfit(2), ...
%     Fit.pfit(4));
